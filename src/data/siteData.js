export const companyInfo = {
  name: 'VELVAN AGRO CENTRE',
  location: 'Karumathampatti, Coimbatore',
  established: 1972,
  founder: 'K.P. Palanisamy',
  managingDirector: 'Krishna Prabhu Palanisamy',
  mdSince: 2009,
  tagline: 'Growing together with modern agriculture solutions',
  about:
    'VELVAN AGRO CENTRE is a trusted agribusiness partner offering fertilizer, pesticide, fungicide, herbicide, and seed solutions to farmers and growers in Tamil Nadu. Established in 1972, we continue to support modern farming with quality products and expert guidance.',
};

export const divisions = [
  {
    name: 'Fertilizers',
    image:
      'https://5.imimg.com/data5/SELLER/Default/2023/6/316955617/GK/CM/QR/150731759/water-soluble-fertilizers-1000x1000.jpeg',
  },
  {
    name: 'Pesticides',
    image:
      'https://th.bing.com/th/id/OIP.9e50hGJFv779FFZluVEnYwHaE7',
  },
  {
    name: 'Fungicides',
    image:
      'https://www.upl-ltd.com/images/fungicide/UPL-Fungicide-Landing-Image.jpg',
  },
  {
    name: 'Herbicides',
    image:
      'https://th.bing.com/th/id/OIP.UHaGoa9dW6pXDCap41rHyAHaE8?o=7rm=3&rs=1&pid=ImgDetMain&o=7&rm=3',
  },
  {
    name: 'Seeds',
    image:
      'https://img.freepik.com/premium-photo/farmer-sowing-seeds-fertile-soil-sunny-day-beginning-process-planting-crops-bountiful-harvest-season_1162141-14998.jpg?w=2000',
  },
]

export const dealerships = {
  fertilizer:[
    {
      name: 'MADRAS FERTILIZERS LTD',
      image: 'https://questionpapershub.com/free-job-alert/wp-content/uploads/2021/02/mfl-recruitment-2021-1068x690.png',
    },
    {
      name: 'IFFCO LTD',
      image: 'https://tse3.mm.bing.net/th/id/OIP.bgvUx9dWCSCRkLC-XNkTJwAAAA?rs=1&pid=ImgDetMain&o=7&rm=3',
    },
    {
      name: 'KRIBHCO LTD',
      image: 'https://www.icexindia.com/images/default-source/partners/kribhco_logo.jpg',
    },
    {
      name: 'MADRAS FERTILIZERS LTD',
      image: 'https://questionpapershub.com/free-job-alert/wp-content/uploads/2021/02/mfl-recruitment-2021-1068x690.png',
    },
  ],

  waterSoluble: [
    {
      name: 'YARA FERTILIZERS LTD',
      image: 'https://th.bing.com/th/id/OIP.zu_9ABgwAsUel0Bte9qZswHaHa?o=7rm=3&rs=1&pid=ImgDetMain&o=7&rm=3',
    },
    {
      name: 'MADRAS FERTILIZERS LTD',
      image: 'https://questionpapershub.com/free-job-alert/wp-content/uploads/2021/02/mfl-recruitment-2021-1068x690.png',
    },
    {
      name: 'ICL - ISRAEL CHEMICALS LTD',
      image: 'https://th.bing.com/th/id/R.707d217363f20924bbd48cb828424ca0?rik=vp2g%2bwA85iYRKA&riu=http%3a%2f%2fi.forbesimg.com%2fmedia%2flists%2fcompanies%2ficl-israel-chemicals_416x416.jpg&ehk=3nJE4tTO38tD1yHGPi2XIFYKBfWxZsuvTQ8jb5ArtdM%3d&risl=&pid=ImgRaw&r=0',
    },
    {
      name: 'KRIBHCO LTD',
      image: 'https://www.icexindia.com/images/default-source/partners/kribhco_logo.jpg',
    },
  ],

  pesticide: [
    {
      name: 'COROMANDEL INTERNATIONAL LTD',
      image: 'https://kj1bcdn.b-cdn.net/media/98821/coromandel.jpg',
    },
    {
      name: 'GHARDA CHEMICALS LTD',
      image: 'https://tse3.mm.bing.net/th/id/OIP.iKCQRuh4OxpRmx1Dl5DsCgHaED?rs=1&pid=ImgDetMain&o=7&rm=3',
    },
    {
      name: 'MADRAS FERTILIZERS LTD',
      image: 'https://questionpapershub.com/free-job-alert/wp-content/uploads/2021/02/mfl-recruitment-2021-1068x690.png',
    },
    {
      name: 'KRIBHCO LTD',
      image: 'https://www.icexindia.com/images/default-source/partners/kribhco_logo.jpg',
    },
  ],
};
